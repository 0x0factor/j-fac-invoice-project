...
var FormClass = function () {
    ...
    this.f_init = function () {
        ...
        this.subtotal = 0;
        this.reg1 = new RegExp(
            "^data\\[([0-9]+)\\]\\[" +
                this.type +
                "\\]\\[(QUANTITY|UNIT_PRICE|DISCOUNT)\\]$"
        );
        this.reg2 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[(FRACTION|EXCISE)\\]$"
        );
        this.reg3 = new RegExp(
            "^data\\[([0-9]+)\\]\\[" + this.type + "\\]\\[(UNIT_PRICE)\\]$"
        );
        this.reg4 = new RegExp(
            "^data\\[([0-9]+)\\]\\[" + this.type + "\\]\\[NOTE_CHECK\\]$"
        );
        this.reg5 = new RegExp(
            "^data\\[([0-9]+)\\]\\[" + this.type + "\\]\\[DISCOUNT_TYPE\\]$"
        );
        this.reg6 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[DISCOUNT_TYPE\\]$"
        );
        this.reg7 = new RegExp(
            "^data\\[[0-9]+\\]\\[" + this.type + "\\]\\[(.+)\\]$"
        );
        this.reg8 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[(CUTOOFF_SELECT)\\]$"
        );
        this.reg9 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[(PAYMENT_SELECT)\\]$"
        );
        this.reg10 = new RegExp("^data\\[([0-9]+)\\]\\[" + this.type + "\\]");
        this.reg12 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[(DISCOUNT)\\]$"
        );
        this.reg13 = new RegExp(
            "^data\\[([0-9]+)\\]\\[" + this.type + "\\]\\[(QUANTITY)\\]$"
        );
        this.reg14 = new RegExp(
            "^data\\[([0-9]+)\\]\\[" +
                this.type +
                "\\]\\[(UNIT_PRICE|AMOUNT)\\]$"
        );
        this.reg15 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[DECIMAL_QUANTITY\\]$"
        );
        this.reg16 = new RegExp(
            "^data\\[" + this.maintype + "\\]\\[DECIMAL_UNITPRICE\\]$"
        );
        ...
        //
        this.maxformline = $(
            'input[name="data[' + this.maintype + '][maxformline]"]'
        ).val();
        this.dataformline = $(
            'input[name="data[' + this.maintype + '][dataformline]"]'
        ).val()
            ? $(
                  'input[name="data[' + this.maintype + '][dataformline]"]'
              ).val() * 1
        ...
        for (var i = 0; i < this.dataformline; i++) {
            if (
                $(
                    'input[name="data[' + i + "][" + this.type + '][DISCOUNT]"]'
                ).val()
            ) {
        ...
        if (
            $("tr.row_0 td img.delbtn") &&
            $("tr.row_0 td img.add_0") &&
            $("tr.row_0 td img.del_0")
        ) {
        ...
        function KeyEvent(e) {
            e = e || window.event;
            pressKey = e.keyCode;
            if (pressKey == 13 && form.textarea != 1 && enterKeyGo == 0) {
                return false;
            }
        }
        ...
        if (
            $('div[id="billfrag"]').text() == "1" ||
            $('div[id="edit_stat"]').text() == "0"
        ) {
            $(".hidebox_d").css("display", "none");
            $(".hidebox_s").css("display", "true");
        }
        if (
            $('div[id="billfrag"]').text() == "0" ||
            $('div[id="edit_stat"]').text() == "1"
        ) {
            $(".hidebox").html("");
        ...
        if (
            ...
            $('input[name="data[Bill][TOTAL]"]').val(
                form.number_format($('input[name="data[Bill][TOTAL]"]').val())
            );
            $(".imgcheck").mouseover(function () {
                $('input[name="data[Bill][TOTAL]"]').val(
                    form.unnumber_format(
                        $('input[name="data[Bill][TOTAL]"]').val()
                    )
                );
            });
        ...
        if (
            ...
            $("form")
                .attr("class")
                .match(/^(Quote|Bill|Delivery|Regularbill)$/)
        ) {
            //初期状態のチェック
            if (
                $(
                ...
                $(
                    'input[name="data[' + form.maintype + '][DISCOUNT]"]'
                ).removeAttr("readonly");
            }
            if (
                $(
                ...
                $('input[name="data[' + form.maintype + '][DISCOUNT]"]').attr(
                    "maxlength",
                    3
                );
            }
            ...
            $("#" + form.maintype + "CSTID").change(function () {
                if ($(this).val().match("^[0-9]+$")) {
                    ...
                    if (param[0]) {
                        $(
                            'input[name="data[' + form.maintype + '][EXCISE]"]'
                        ).val([param[0]]);
                    }
                    if (param[1]) {
                        $(
                    ...
                    var param = $(
                        'input[name="data[' + form.maintype + '][default]"]'
                    )
                        .val()
                        .split("-");
                    $('input[name="data[' + form.maintype + '][EXCISE]"]').val([
                        param[0],
                    ]);
                    $(
                        'input[name="data[' + form.maintype + '][FRACTION]"]'
                    ).val([param[1]]);
                    form.f_subtotal();
                }
            });
            ...
            $('input[type="radio"]').click(function () {
                ...
                if ((match = $(this).attr("name").match(form.reg5))) {
                    ...
                    if ($(this).val() != 2) {
                        ...
                        for (
                            var i = 0;
                            i < this.dataformline + form.nowformline;
                            i++
                        ) {
                            var line_attribute = $(
                                'select[name="data[' +
                    ...
                    for (
                        ...
                        if (
                            ...
                            var quantity = form.unnumber_format(
                                $(
                                    'input[name="data[' +
                                        i +
                                        "][" +
                    ...
                    for (
                        ...
                        if (
                            ...
                            var unitprice = form.unnumber_format(
                                $(
                                    'input[name="data[' +
                                        i +
                                        "][" +
                            ...
                            if (stnum != -1) {
                                $(
                                    'input[name="data[' +
                                        ...
                                        form.type +
                                        '][UNIT_PRICE]"]'
                                ).val(fix_fraction(unitprice, dec));
                            }
                        }
                    }
                }
                            ...
                            if (stnum != -1) {
                                ...
                                $(
                                    'input[name="data[' +
                                        ...
                                        form.type +
                                        '][UNIT_PRICE]"]'
                                ).val(form.number_format(unitprice));
                            }
                            //数量、単価入力済み
                            if (
                                $(
                            ...
                            if (stnum != -1) {
                                ...
                                $(
                                    'input[name="data[' +
                                        ...
                                        form.type +
                                        '][QUANTITY]"]'
                                ).val(form.number_format(quantity));
                            }
                            //数量、単価入力済み
                            if (
                                $(
                ...
                .submit(function () {
                    ...
                    $("input").each(function () {
                        if ((match = $(this).attr("name").match(form.reg14))) {
                            ...
                            (match = $(this).attr("name").match(form.reg13))
                        ) {
                            $(this).val(form.unnumber_format($(this).val()));
                        } else if (
                            $(this)
            ...
            $('input[name="data[' + form.maintype + '][SUBTOTAL]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' + form.maintype + '][SUBTOTAL]"]'
                    ).val()
                )
            );
            $('input[name="data[' + form.maintype + '][SALES_TAX]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' + form.maintype + '][SALES_TAX]"]'
                    ).val()
                )
            );
            $('input[name="data[' + form.maintype + '][TOTAL]"]').val(
                form.number_format(
                    $('input[name="data[' + form.maintype + '][TOTAL]"]').val()
                )
            );
            $('input[name="data[' + form.maintype + '][FIVE_RATE_TAX]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' +
                            form.maintype +
            ...
            $('input[name="data[' + form.maintype + '][FIVE_RATE_TOTAL]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' +
                            form.maintype +
            ...
            $('input[name="data[' + form.maintype + '][EIGHT_RATE_TAX]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' +
                            form.maintype +
            ...
            $(
                'input[name="data[' + form.maintype + '][EIGHT_RATE_TOTAL]"]'
            ).val(
                form.number_format(
                    $(
                        'input[name="data[' +
            ...
            $(
                'input[name="data[' + form.maintype + '][REDUCED_RATE_TAX]"]'
            ).val(
                form.number_format(
                    $(
                        'input[name="data[' +
            ...
            $(
                'input[name="data[' + form.maintype + '][REDUCED_RATE_TOTAL]"]'
            ).val(
                form.number_format(
                    $(
                        'input[name="data[' +
            ...
            $('input[name="data[' + form.maintype + '][TEN_RATE_TAX]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' +
                            form.maintype +
            ...
            $('input[name="data[' + form.maintype + '][TEN_RATE_TOTAL]"]').val(
                form.number_format(
                    $(
                        'input[name="data[' +
                            form.maintype +
        ...
        if (
            ...
            if (
                $(
                    'input[name="data[' +
                        form.maintype +
                        '][CUTOOFF_SELECT]"]:checked'
                ).val() == 0
            ) {
                $(
                    'input[name="data[' + form.maintype + '][CUTOOFF_DATE]"]'
            ...
            if (
                $(
                    'input[name="data[' +
                        form.maintype +
                        '][PAYMENT_SELECT]"]:checked'
                ).val() == 0
            ) {
                $(
                    'input[name="data[' + form.maintype + '][PAYMENT_DAY]"]'
                ).attr("readonly", "readonly");
            }
        ...
        if (
            ...
            var reg11 = new RegExp(
                "^data\\[" + this.maintype + "\\]\\[(UNIT_PRICE)\\]$"
            );

            $(".imgcheck").mouseover(function () {
                $('input[name="data[' + form.maintype + '][UNIT_PRICE]"]').val(
                    form.unnumber_format(
                        $(
                            'input[name="data[' +
                                form.maintype +
                ...
                .blur(function () {
                    if (!form.CHcheck) {
                        ...
                        if ((match = $(this).attr("name").match(reg11))) {
                            ...
                            if (stnum != -1) {
                                if (
                                    unitprice.toString().slice(stnum + 1)
                                        .length > dec
                                ) {
                                    if (Number(unitprice)) {
                                        $(
        ...
        if (
            ...
            if (
                $('select[name="data[' + form.maintype + '][STATUS]"]').val() ==
                0
            ) {
                $("div.Smtpuse").hide();
            }
            if (
                ...
                $('input[name="data[' + form.maintype + '][USER]"]').attr(
                    "readonly",
                    "readonly"
                );
                $('input[name="data[' + form.maintype + '][PASS]"]').attr(
                    "readonly",
                    "readonly"
                );
            }

            $('select[name="data[' + form.maintype + '][STATUS]"]').change(
                function () {
                    if (
                        $(
                            'select[name="data[' + form.maintype + '][STATUS]"]'
                        ).val() == 1
                    ) {
                    ...
                    if (
                        ...
                        $(
                            'input[name="data[' + form.maintype + '][HOST]"]'
                        ).val(null);
                        $(
                            'input[name="data[' + form.maintype + '][PORT]"]'
                        ).val(null);
                        $(
                            'input[name="data[' + form.maintype + '][USER]"]'
                        ).val(null);
                        $(
                            'input[name="data[' + form.maintype + '][PASS]"]'
                        ).val(null);
                    }
                }
            );

            $('input[name="data[' + form.maintype + '][PROTOCOL]"]').change(
                function () {
                    if ($(this).val() == 0) {
                        $(
                            'input[name="data[' + form.maintype + '][USER]"]'
        ...
        if (
            ...
            //初期状態チェック
            $('input[name="data[Totalbill][LASTM_BILL]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][LASTM_BILL]"]').val()
                )
            );
            $('input[name="data[Totalbill][DEPOSIT]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][DEPOSIT]"]').val()
                )
            );
            $('input[name="data[Totalbill][SALE]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][SALE]"]').val()
                )
            );
            $('input[name="data[Totalbill][SALE_TAX]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][SALE_TAX]"]').val()
                )
            );
            $('input[name="data[Totalbill][CARRY_BILL]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][CARRY_BILL]"]').val()
                )
            );
            $('input[name="data[Totalbill][THISM_BILL]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][THISM_BILL]"]').val()
                )
            );
            $('input[name="data[Totalbill][SUBTOTAL]"]').val(
                form.number_format(
                    $('input[name="data[Totalbill][SUBTOTAL]"]').val()
                )
            );
            ...
            if (
                $(
                    'input[name="data[' + form.maintype + '][EDIT_STAT]"]'
                ).val() == "0"
            ) {
                $(".hidebox_d").html("");
                $(".hidebox_s").css("display", "true");
            ...
            $(".hidebox_d input")
                .blur(function () {
                    var match;
                    tb_reg1 = new RegExp(
                        "^data\\[Totalbill\\]\\[(LASTM_BILL)\\]$"
                    );
                    tb_reg2 = new RegExp(
                        "^data\\[Totalbill\\]\\[(DEPOSIT)\\]$"
                    );

                    lastm_bill = form.unnumber_format(
                        $('input[name="data[Totalbill][LASTM_BILL]"]').val()
                    );
                    deposit = form.unnumber_format(
                        $('input[name="data[Totalbill][DEPOSIT]"]').val()
                    );
                    carry_bill = form.unnumber_format(
                        $('input[name="data[Totalbill][CARRY_BILL]"]').val()
                    );
                    sale = form.unnumber_format(
                        $('input[name="data[Totalbill][SALE]"]').val()
                    );
                    sale_tax = form.unnumber_format(
                        $('input[name="data[Totalbill][SALE_TAX]"]').val()
                    );
                    thism_bill = form.unnumber_format(
                        $('input[name="data[Totalbill][THISM_BILL]"]').val()
                    );

                    if (
                        lastm_bill.match(/^-?[0-9]+$/) != null &&
                        deposit.match(/^-?[0-9]+$/) != null &&
                        sale.match(/^-?[0-9]+$/) != null &&
                        sale_tax.match(/^-?[0-9]+$/) != null
                    ) {
                        carry_bill = lastm_bill - deposit;
                        thism_bill = Number(sale) + Number(carry_bill);
                    }
                    $('input[name="data[Totalbill][LASTM_BILL]"]').val(
                        form.number_format(lastm_bill)
                    );
                    $('input[name="data[Totalbill][DEPOSIT]"]').val(
                        form.number_format(deposit)
                    );
                    $('input[name="data[Totalbill][SALE]"]').val(
                        form.number_format(sale)
                    );
                    $('input[name="data[Totalbill][SALE_TAX]"]').val(
                        form.number_format(sale_tax)
                    );
                    $('input[name="data[Totalbill][CARRY_BILL]"]').val(
                        form.number_format(carry_bill)
                    );
                    $('input[name="data[Totalbill][THISM_BILL]"]').val(
                        form.number_format(thism_bill)
                    );
                })
                .focus(function () {
                    $(this).val(form.unnumber_format($(this).val()));
            ...
            $(".hidebox_s input")
                .blur(function () {
                    ...
                    subtotal = form.unnumber_format(
                        $('input[name="data[Totalbill][SUBTOTAL]"]').val()
                    );

                    if (
                        subtotal.match(/^-?[0-9]+$/) != null &&
                        sale_tax.match(/^-?[0-9]+$/) != null &&
        ...
        $(".nowtime").click(function () {
            ...
            var sdate =
                year.toString() + "-" + mon.toString() + "-" + today.toString();
        ...
        if (
            $(
                'input[name="data[' + this.maintype + '][FIVE_RATE_TOTAL]"]'
            ).val() == 0
        ) {
        ...
        if (
            $(
                'input[name="data[' + this.maintype + '][EIGHT_RATE_TOTAL]"]'
            ).val() == 0
        ) {
        ...
        if (
            $(
                'input[name="data[' + this.maintype + '][REDUCED_RATE_TOTAL]"]'
            ).val() == 0
        ) {
        ...
        if (
            $(
                'input[name="data[' + this.maintype + '][TEN_RATE_TOTAL]"]'
            ).val() == 0
        ) {
    ...
    this.discount_all = function (total, fraction) {
        var all_dis_amount = Number(
            this.unnumber_format(
                $('input[name="data[' + this.maintype + '][DISCOUNT]"]').val()
            )
        );
        if (
            $(
                'input[name="data[' +
                    this.maintype +
                    '][DISCOUNT_TYPE]"]:radio:checked'
            ).val() == 0
        ) {
            total = this.f_fraction(
                fraction,
                (Number(total) * (100 - Number(all_dis_amount))) / 100
            );
        } else if (
            $(
                'input[name="data[' +
    ...
    this.f_total = function () {
        ...
        // 端数処理・税区分
        const fraction = $(
            'input[name="data[' + this.maintype + '][FRACTION]"]:checked'
        ).val();
        const taxFraction = $(
            'input[name="data[' + this.maintype + '][TAX_FRACTION]"]:checked'
        ).val();
        const taxFractionTiming = $(
            'input[name="data[' +
                this.maintype +
                '][TAX_FRACTION_TIMING]"]:checked'
        ).val();
        ...
        for (var i = 0; true; i++) {
            ...
            if (
                !$(
                    'input[name="data[' + i + "][" + this.type + '][AMOUNT]"]'
                ).attr("name")
            ) {
                break;
            }
            ...
            for (let i in taxTypes) {
                ...
                if (amounts[nowTax].length > 0) {
                    ...
                    for (let k in amounts[nowTax]) {
                        ...
                        if (
                            line["inTaxFlg"] == true &&
                            !taxTypes.includes("intax")
                        ) {
                            taxTypes.push("intax");
                        } else if (
                            line["inTaxFlg"] == false &&
                            !taxTypes.includes("outtax")
                        ) {
                    ...
                    if (taxTypes.length == 1 && taxTypes.includes("outtax")) {
                        ...
                        if (taxTypes.includes("outtax")) {
                            ...
                            outTaxTotal = this.discount_all(
                                outTaxTotal,
                                fraction
                            );
                            const outTax = this.f_fraction(
                                taxFraction,
                                (outTaxTotal * nowTagInteger) / 100
                            );

                            eachTaxTotal[nowTax]["amount"] =
                                outTaxTotal + outTax;
                            eachTaxTotal[nowTax]["tax"] = outTax;
                            eachTaxTotal[nowTax]["count"] =
                                amounts[nowTax].length;
                            // 内税の場合
                            ...
                            eachTaxTotal[nowTax]["tax"] = this.f_fraction(
                                taxFraction,
                                (eachTaxTotal[nowTax]["amount"] *
                                    nowTagInteger) /
                                    (nowTagInteger + 100)
                        ...
                        const outTaxTotalWithTotal = this.f_fraction(
                            fraction,
                            (outTaxTotal * (nowTagInteger + 100)) / 100
                        );

                        eachTaxTotal[nowTax]["amount"] =
                            inTaxTotal + outTaxTotalWithTotal;
                        eachTaxTotal[nowTax]["amount"] = this.discount_all(
                            eachTaxTotal[nowTax]["amount"],
                            fraction
                        );
                        const taxAmount =
                            (eachTaxTotal[nowTax]["amount"] * nowTagInteger) /
                            (nowTagInteger + 100);
                        eachTaxTotal[nowTax]["tax"] = this.f_fraction(
                            taxFraction,
                            taxAmount
                        ...
                        if (
                            Math.floor(taxAmount) <= eachTaxTotal[nowTax]["tax"]
                        ) {
                            eachTaxTotal[nowTax]["amount"] +=
                                eachTaxTotal[nowTax]["tax"] -
            ...
            for (let i in taxTypes) {
                ...
                if (amounts[nowTax].length > 0) {
                    for (let k in amounts[nowTax]) {
                        ...
                        if (line["inTaxFlg"] == true) {
                            const calcTax = this.f_fraction(
                                taxFraction,
                                (line["amount"] * nowTagInteger) /
                                    (100 + +nowTagInteger)
                            );
        ...
        $('input[name="data[' + this.maintype + '][TOTAL]"]').val(
            this.number_format(total)
        );
        $('input[name="data[' + this.maintype + '][SALES_TAX]"]').val(
            this.number_format(taxTotal)
        );
        $('input[name="data[' + this.maintype + '][SUBTOTAL]"]').val(
            this.number_format(beforeTax)
        );

        // 5%
        $('input[name="data[' + this.maintype + '][FIVE_RATE_TAX]"]').val(
            this.number_format(eachTaxTotal["5"]["tax"])
        );
        $('input[name="data[' + this.maintype + '][FIVE_RATE_TOTAL]"]').val(
            this.number_format(eachTaxTotal["5"]["amount"])
        );
        // 8%
        $('input[name="data[' + this.maintype + '][EIGHT_RATE_TAX]"]').val(
            this.number_format(eachTaxTotal["8"]["tax"])
        );
        $('input[name="data[' + this.maintype + '][EIGHT_RATE_TOTAL]"]').val(
            this.number_format(eachTaxTotal["8"]["amount"])
        );
        // 8%軽減
        $('input[name="data[' + this.maintype + '][REDUCED_RATE_TAX]"]').val(
            this.number_format(eachTaxTotal["r8"]["tax"])
        );
        $('input[name="data[' + this.maintype + '][REDUCED_RATE_TOTAL]"]').val(
            this.number_format(eachTaxTotal["r8"]["amount"])
        );
        // 10%
        $('input[name="data[' + this.maintype + '][TEN_RATE_TAX]"]').val(
            this.number_format(eachTaxTotal["10"]["tax"])
        );
        $('input[name="data[' + this.maintype + '][TEN_RATE_TOTAL]"]').val(
            this.number_format(eachTaxTotal["10"]["amount"])
        );
    ...
    this.f_row = function (_no) {
        ...
        if (
            $(
                'input[name="data[' + _no + "][" + this.type + '][QUANTITY]"]'
            ).val() == ""
        ) {
            amount = "";
        }
        if (
            $(
                'input[name="data[' + _no + "][" + this.type + '][UNIT_PRICE]"]'
            ).val() == ""
        ) {
            amount = "";
        }

        $('input[name="data[' + _no + "][" + this.type + '][AMOUNT]"]')
            .val(this.number_format(amount))
            .css("color", "#000");
        this.f_subtotal();
    };
    ...
    this.number_format = function (_num) {
        ...
        if (
            ...
            !_num
                .toString()
                .match(/^(\\|\$)?(0|-?[1-9]\d*|-?(0|[1-9]\d*)\.\d+)$/)
        ) {
            return _num;
        }
        for (i = 0; _num.toString().charAt(0) == "0"; i++) {
            if (
                _num.toString().length == 1 ||
                _num.toString().charAt(1) == "."
            ) {
                break;
            }
    ...
    this.f_cleateline = function (_no, _val) {
        ...
        html =
            '<td><a href="#"><img src="' +
            this.bt_del +
            '" alt="×" onclick="return form.f_delline(' +
            _no +
            ');"><a></td>';
        html +=
            ...
            '][ITEM_NO]" ' +
            'value="' +
            (_val["ITEM_NO"] ? _val["ITEM_NO"] : "") +
            '" ' +
            'class="w31" maxlength="2"></td>';
        html +=
            '<td><input type="text" id="' +
            this.maintype +
            'Name" name="data[' +
            ...
            '][ITEM_CODE]" ' +
            'value="' +
            (_val["ITEM_CODE"] ? _val["ITEM_CODE"] : "") +
            '" ' +
            'class="w64" maxlength="8"></td>';
        html +=
            '<td><input type="text" id="' +
            this.maintype +
            'Name" name="data[' +
            ...
            (_val["ITEM"] ? _val["ITEM"] : "") +
            '" ' +
            'class="w120" maxlength="25" >';
        html +=
            ' <span id="INSERT_ITEM_IMG' +
            _no +
            '" ><a href="#"><img src="' +
            bt_select +
            '" alt="商品選択" onclick="form.focusline = ' +
            _no +
            ';focusLine();return popupclass.popupajax(\'select_item\');" style="margin: 0px 0px 2px;"/></a></span></td>';
        html +=
            '<td><input type="text" id="' +
            this.maintype +
            'Name" name="data[' +
            ...
            '][QUANTITY]" ' +
            'value="' +
            (_val["QUANTITY"] ? _val["QUANTITY"] : "") +
            '" ' +
            'class="w63" maxlength="7" onkeyup="recalculation(\'' +
            this.maintype +
            "');\"></td>";
        html +=
            '<td><input type="text" id="' +
        ...
        html +=
            ...
            '][UNIT_PRICE]" ' +
            'value="' +
            (_val["UNIT_PRICE"] ? _val["UNIT_PRICE"] : "") +
            '" ' +
            'class="w73" maxlength="9" onkeyup="recalculation(\'' +
            this.maintype +
            "');\"></td>";
        html +=
            '<td><input type="text" id="' +
            ...
            '][AMOUNT]" ' +
            'value="' +
            (_val["AMOUNT"] ? _val["AMOUNT"] : "") +
            '" ' +
            'class="w103" readonly="" onChange="recalculation(\'' +
            this.maintype +
            "');\" onkeyup=\"recalculation('" +
            this.maintype +
            "');\"></td>";
        html +=
            '<td><select name="data[' +
            ...
            "][" +
            this.type +
            '][LINE_ATTRIBUTE]" class="w103" onChange="changeAttribute(\'' +
            this.maintype +
            "'," +
            _no +
        ...
        html +=
            ...
            "][" +
            this.type +
            '][TAX_CLASS]" class="w105" onchange="changeTaxClass(\'' +
            this.maintype +
            "'," +
            _no +
            ', value);" style="display: inline">';
                ...
                if ((match = this_name.match(form.reg1))) {
                    ...
                    $(this)
                        ...
                        .focus(function () {
                            var match;
                            if (
                                (match = $(this).attr("name").match(form.reg3))
                            ) {
                                $(
                                    'input[name="data[' +
                                        match[1] +
    ...
    this.f_addline = function (_no) {
        ...
        if (_no != null) {
            ...
            $("tr.row_" + (this.nowformline - 1)).after(
                this.f_cleateraw(this.nowformline)
            );
            $("tr.row_" + this.nowformline).html(
                this.f_cleateline(this.nowformline, new Array())
            );
        }
                        ...
                        if (!form.CHcheck) {
                            ...
                            if (
                                ...
                                if (stnum != -1) {
                                    ...
                                    if (chconma == -1) {
                                        if (
                                            quantity.toString().slice(stnum + 1)
                                                .length > dec
                                        ) {
                                            if (Number(quantity)) {
                                                $(
    ...
    this.coverpage_addline = function (_no) {
        if (this.nowformline + 1 < this.maxformline) {
            ...
            html +=
                '<td style="width: 42px;"><a href="#"><img src="' +
                this.bt_del +
                '" alt="×" onclick="return form.coverpage_delline(' +
                row_num +
                ');"></a></td>';
            html +=
                '<td style="width: 250px;"><input class="documenttitle" type="text" style="width: 250px;" maxlength = "15" name="data[' +
                row_num +
                '][Reports][DOCUMENT_TITLE]"></td>';
            html +=
                '<td style="width: 100px;"><input class="documentnumber" type="text" style="width: 80px;" maxlength = "7" name="data[' +
                row_num +
                '][Reports][DOCUMENT_NUMBER]">&nbsp;部</td>';
    ...
    this.coverpage_shiftline = function (_no) {
        for (var i = _no; i < this.maxformline - 1; i++) {
            $('tr[class="row_' + i + '"] input[class="documenttitle"]').val(
                $(
                    'tr[class="row_' +
                        (i + 1) +
                        '"] input[class="documenttitle"]'
                ).val()
            );
            $('tr[class="row_' + i + '"] input[class="documentnumber"]').val(
                $(
                    'tr[class="row_' +
                        (i + 1) +
                        '"] input[class="documentnumber"]'
                ).val()
            );
        }
        return false;
    ...
    this.coverpage_delline = function (_no) {
        if (this.nowformline > 0) {
            ...
            $('tr[class="row_' + this.nowformline-- + '"]').css({
                display: "none",
            });
        }
        return false;
            ...
            if (stnum != -1) {
                if (unitprice.toString().slice(stnum + 1).length > dec) {
                    $(
                        ...
                        .css("color", "#000")
                        .val(fix_fraction(data[itemno].UNIT_PRICE, dec));
                    ...
                    $(
                        ...
                        .css("color", "#000")
                        .val(this.number_format(data[itemno].UNIT_PRICE));
                }
                ...
                $(
                    ...
                    .css("color", "#000")
                    .val(this.number_format(Number(data[itemno].UNIT_PRICE)));
                form.f_row(form.focusline);
            }
        }
        setReadOnly(form.maintype);
    ...
    this.f_up = function (insert) {
        ...
        if (form.focusline > 0) {
            ...
            for (
                var i = startRow;
                i < detailTable.rows[1].cells.length - 1;
                i++
            ) {
                if (i == 8) {
                    temp =
                        detailTable.rows[
                            eval(startRow) + eval(form.focusline) + 1
                        ].cells[i].getElementsByTagName("select")[0].value;
                    detailTable.rows[
                        eval(startRow) + eval(form.focusline) + 1
                    ].cells[i].getElementsByTagName("select")[0].value =
                        detailTable.rows[
                            eval(startRow) + eval(form.focusline)
                        ].cells[i].getElementsByTagName("select")[0].value;
                    detailTable.rows[
                        eval(startRow) + eval(form.focusline)
                    ].cells[i].getElementsByTagName("select")[0].value = temp;

                    temp =
                        detailTable.rows[
                            eval(startRow) + eval(form.focusline) + 1
                        ].cells[i].getElementsByTagName("select")[1].value;
                    detailTable.rows[
                        eval(startRow) + eval(form.focusline) + 1
                    ].cells[i].getElementsByTagName("select")[1].value =
                        detailTable.rows[
                    ...
                    detailTable.rows[
                        eval(startRow) + eval(form.focusline)
                    ].cells[i].getElementsByTagName("select")[1].value = temp;
                ...
                detailTable.rows[eval(startRow) + eval(form.focusline)].cells[
                    i
                ].getElementsByTagName("input")[0].value = temp;
            }
        ...
        if (form.focusline < form.nowformline) {
            ...
            for (
                ...
                if (i == 8) {
                    temp =
                        detailTable.rows[
                            eval(startRow) + eval(form.focusline) + 2
                        ].cells[i].getElementsByTagName("select")[0].value;
                    detailTable.rows[
                        eval(startRow) + eval(form.focusline) + 2
                    ].cells[i].getElementsByTagName("select")[0].value =
                ...
                detailTable.rows[
                    eval(startRow) + eval(form.focusline) + 1
                ].cells[i].getElementsByTagName("input")[0].value = temp;
            }
...
function count_str(id, str, max) {
    if (max - str.length >= 0) {
        $("#" + id)
            .html("(あと" + (max - str.length) + "文字)")
            .removeClass("str_rest");
    } else {
        $("#" + id)
            .addClass("str_rest")
            .html(max + "文字を超えています");
...
function focusLine(type) {
    ...
    if (ie == 8) {
        $(
            ':text[name*="data[' + form.focusline + ']"][readonly="readonly"]'
        ).addClass("focusReadOnly");
    } else {
        $(':text[name*="data[' + form.focusline + ']"][readonly=""]').addClass(
            "focusReadOnly"
        );
        $(':text[name*="data[' + form.focusline + ']"][readonly]').addClass(
            "focusReadOnly"
...
function setReadOnly(type) {
    ...
    for (var i = 0; i <= form.nowformline; i++) {
        tempAttribute = $(
            'select[name="data[' + i + "][" + type + 'item][LINE_ATTRIBUTE]"]'
        ).val();
        ...
        switch (Number(tempAttribute)) {
            case 0: //通常
                ...
                $(
                    'input[name="data[' + i + "][" + type + 'item][AMOUNT]"]'
                ).attr("readonly", "readonly");
                break;
            ...
            case 3: //割引(円)
                addReadOnly(type, i);
                $(
                    'input[name="data[' + i + "][" + type + 'item][ITEM]"]'
                ).removeAttr("readonly");
                $(
                    'input[name="data[' + i + "][" + type + 'item][AMOUNT]"]'
                ).removeAttr("readonly");
            ...
            case 4: //割引(％)
                ...
                $(
                    'input[name="data[' + i + "][" + type + 'item][QUANTITY]"]'
                ).removeAttr("readonly");
                break;
...
function setTaxClass(type, no, value) {
    var excise = $(
        'input[name="data[' + type + '][EXCISE]"]:radio:checked'
    ).val();

    switch (Number(value)) {
        case 0: //通常時処理
            $(
                'select[name="data[' + no + "][" + type + 'item][TAX_CLASS]"]'
            ).val(eval(Number(excise) + 1));
            break;
...
function changeTaxClass(type, no, value) {
    var tmpAttr = $(
        'select[name="data[' + no + "][" + type + 'item][LINE_ATTRIBUTE]"]'
    ).val();
    var excise = $(
        'input[name="data[' + type + '][EXCISE]"]:radio:checked'
    ).val();
...
function changeAttribute(type, no, value) {
    switch (Number(value)) {
        ...
        case 3: //割引(円)
            ...
            setReadOnly(type);
            setTaxClass(type, no, value);
            $('input[name="data[' + no + "][" + type + 'item][ITEM]"]').val(
                "　(割引)"
            );
            $('input[name="data[' + no + "][" + type + 'item][AMOUNT]"]').val(
                "-0"
            );
...
function calcTax(type, no, attr, taxClass, discountFlg) {
    ...
    if (discountFlg) {
        ...
        if (taxClass.match(/1$/)) {
            return (
                ((amount + discount) * tax_rate * 100) / (tax_rate * 100 + 100)
            );
            //外税
...
function fix_fraction(num, dec) {
    return (
        Math.floor(Math.multiply(num, Math.pow(10, dec))) / Math.pow(10, dec)
    );
}
...
function check_if_searched() {
    ...
    if ($("#" + controller_name + "ITEMNAME").val() != undefined) {
        if (
            ...
            $("#" + controller_name + "ITEMNAME").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "ITEMCODE").val() != "" &&
            $("#" + controller_name + "ITEMCODE").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "TOTALFROM").val() != "" &&
            $("#" + controller_name + "TOTALFROM").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "TOTALTO").val() != "" &&
            $("#" + controller_name + "TOTALTO").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "NOTE").val() != "" &&
            $("#" + controller_name + "NOTE").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "MEMO").val() != "" &&
            $("#" + controller_name + "MEMO").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "ACTIONDATEFROM").val() != "" &&
            $("#" + controller_name + "ACTIONDATEFROM").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $("#" + controller_name + "ACTIONDATETO").val() != "" &&
            $("#" + controller_name + "ACTIONDATETO").val() != undefined
        ) {
            open_flg = 1;
        }
        if (
            $(
                'input[name="data[' + controller_name + '][ISSUE]"]:checked'
            ).val() != 3 &&
            $(
                'input[name="data[' + controller_name + '][ISSUE]"]:checked'
            ).val() != undefined
