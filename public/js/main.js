$(document).ready(function ($) {
    if ($("form").attr("class")) {
        ...
        function getaddress() {
            ...
            $.post("/ajax/candidacy", { params: params }, function (d) {
                $("div#target").html(d);
            });
        }
    }
...
function setaddress(postcode) {
    if ($("form").attr("class")) {
        ...
        $.post("/ajax/search", { postcode: postcode }, function (d) {
            var data = eval("(" + d + ")");
            if (data) {
    ...
    if (
        ...
        $("input").keyup(function () {
            ...
            if ((match = this_name.match(reg1))) {
                var loginid = $(
                    'input[name="data[Administer][LOGIN_ID]"]'
                ).val();
                $.post(
                    "/ajax/searchid",
                    { logincode: loginid, usercode: usrid },
                    function (d) {
                        $("div#target").html(d);
                    }
                );
...
var WindowClass = function () {
    ...
    this.getScreenSize = function () {
        var isWin9X =
            navigator.appVersion.toLowerCase().indexOf("windows 98") + 1;
        var isIE =
...
var PopupClass = function (win) {
    ...
    this.init = function (form) {
        ...
        $("#popup-bg").css({
            ...
            height: win.getBrowserHeight() + "px",
            top: "0px",
            left: "0px",
        ...
        $(window).resize(function () {
            if ($("#popup-bg").is(":visible") && $("#popup").is(":visible")) {
                ...
                $("#popup").animate(
                    {
                        left:
                            win.getScrollPosition().x +
                            win.getScreenSize().mx -
                            $("#popup").width() / 2 +
                            "px",
                    },
                    { duration: 300, queue: false }
        ...
        $(window).scroll(function () {
            if ($("#popup-bg").is(":visible") && $("#popup").is(":visible")) {
                var heightpoint =
                    win.getBrowserHeight() - 100 - $("#popup").height();
    ...
    this.popup_discount = function (no, type) {
        ...
        var value = $(
            'input[name="data[' + no + "][" + type + '][DISCOUNT]"]'
        ).val();
        var d_type = $(
            'input[name="data[' + no + "][" + type + '][DISCOUNT_TYPE]"]'
        ).val();
        var code =
            "<form>" +
            ...
            "<div>" +
            "<div>" +
            '<img src="/img/popup/tl_sale.jpg" style="padding-bottom:10px;" alt="" />' +
            ...
            '<input type="text" name="p_discount" class="mr10" maxlength="3" value="' +
            value +
            '">' +
            '<input type="text" style="display:none">' +
            '<input type="radio" name="p_discount_type" class="mr5" value="0" ' +
            (d_type == 0 ? 'checked="checked" ' : " ") +
            "/>％" +
            '<input type="radio" name="p_discount_type" class="ml10 mr5" value="1" ' +
            (d_type == 1 ? 'checked="checked" ' : " ") +
            "/>円</td></tr>" +
            '<tr><td colspan="2" class="line">' +
            '<img src="/img/popup/i_line_solid.gif" alt="" />' +
            "</td></tr>" +
            "</table>" +
            "</div></div>" +
            "<div>" +
            '<a href="#" onclick="return popupclass.set_discount(' +
            no +
            ", '" +
            type +
            '\')"><img src="/img/bt_save2.jpg" alt="" /></a>' +
            '<a href="#" alt="キャンセル" onclick="return popupclass.popup_close();"><img src="/img/bt_cancel_s.jpg" alt="" /></a>' +
            "</div>" +
            "</form>";
        ...
        $("#popup")
            ...
            .focus(function () {
                if ($(this).attr("name") == "p_discount") {
                    $('input[name="p_discount"]').val(
                        unnumber_format($('input[name="p_discount"]').val())
                    );
                }
            });
        $("#popup")
        ...
        $("#popup")
            ...
            .blur(function () {
                if ((match = $(this).attr("name") == "p_discount")) {
                    $('input[name="p_discount"]').val(
                        number_format($('input[name="p_discount"]').val())
                    );
                }
            });
        return false;
    ...
    this.del_discount = function (no, type) {
        if (
            !$(
                'input[name="data[' + no + "][" + type + '][DISCOUNT_DISPLAY]"]'
            ).val() == ""
        ) {
            $(
                'input[name="data[' + no + "][" + type + '][DISCOUNT_DISPLAY]"]'
    ...
    this.set_discount = function (no, type) {
        ...
        if ($('input[name="p_discount_type"]:radio:checked').val() == 0) {
            ...
            if ($('input[name="p_discount"]').val() == "") {
                var error =
                    '<tr class="p_discount_error"><th></th><td style="padding:5px;text-align:left;"><span class="must">割引率は必須です</span></td></tr>';
                ...
                var error =
                    '<tr class="p_discount_error"><th></th><td style="padding:5px;text-align:left;"><span class="must">割引率は数字のみです</span></td></tr>';
                $("#popup").find(".p_discount_tr").after(error);
                $("#popup").find("input[name=p_discount]").addClass("error");
                $('input[name="p_discount"]').val(
                    number_format($('input[name="p_discount"]').val())
                ...
                var error =
                    '<tr class="p_discount_error"><th></th><td style="padding:5px;text-align:left;"><span class="must">正しい割引率ではありません</span></td></tr>';
                $("#popup").find(".p_discount_tr").after(error);
                $("#popup").find("input[name=p_discount]").addClass("error");
                $('input[name="p_discount"]').val(
                    number_format($('input[name="p_discount"]').val())
                ...
                $(
                    'input[name="data[' +
                        ...
                        type +
                        '][DISCOUNT_TYPE]"]'
                ).val($('input[name="p_discount_type"]:radio:checked').val());

                var str = "【割引】 ";
                str += $('input[name="p_discount"]').val();
                str += "%";

                $('input[name="data[' + no + "][" + type + '][ITEM]"]').val(
                    str
                );
        ...
        if ($('input[name="p_discount_type"]:radio:checked').val() == 1) {
            ...
            if ($('input[name="p_discount"]').val() == "") {
                var error =
                    '<tr class="p_discount_error"><th></th><td style="padding:5px;text-align:left;"><span class="must">割引は必須です</span></td></tr>';
                $("#popup").find(".p_discount_tr").after(error);
                $("#popup").find("input[name=p_discount]").addClass("error");
                $('input[name="p_discount"]').val(
                    number_format($('input[name="p_discount"]').val())
                ...
                var error =
                    '<tr class="p_discount_error"><th></th><td style="padding:5px;text-align:left;"><span class="must">割引は数字のみです</span></td></tr>';
                $("#popup").find(".p_discount_tr").after(error);
                $("#popup").find("input[name=p_discount]").addClass("error");
                $('input[name="p_discount"]').val(
                    number_format($('input[name="p_discount"]').val())
                ...
                var error =
                    '<tr class="p_discount_error"><th></th><td style="padding:5px;text-align:left;"><span class="must">割引が長すぎます</span></td></tr>';
                $("#popup").find(".p_discount_tr").after(error);
                $("#popup").find("input[name=p_discount]").addClass("error");
                $('input[name="p_discount"]').val(
                    number_format($('input[name="p_discount"]').val())
    ...
    number_format = function (_num) {
        if (
            _num &&
            !_num.toString().match(/^(\|$)?(0|-?[1-9]\d*|-?(0|[1-9]\d*)\.\d+)$/)
        ) {
            return _num;
        }
        ...
        $.post("/ajax/popup", { params: param }, function (d) {
            ...
            $("#popup")
                .css({
                    top: win.getScrollPosition().y + 100 + "px",
                    left:
                        win.getScreenSize().mx - $("#popup").width() / 2 + "px",
                })
                .fadeIn("slow");
            $("#popup").html(d);
            ...
            if (type == "item") {
                $("#popup")
                    ...
                    .focus(function () {
                        if ($(this).attr("name") == "data[UNIT_PRICE]") {
                            $('input[name="data[UNIT_PRICE]"]').val(
                                unnumber_format(
                                    $('input[name="data[UNIT_PRICE]"]').val()
                                )
                            );
                ...
                $("#popup")
                    ...
                    .blur(function () {
                        if (
                            (match = $(this).attr("name") == "data[UNIT_PRICE]")
                        ) {
    ...
    this.popupinsert = function (type) {
        ...
        if (type == "item") {
            $('input[name="data[UNIT_PRICE]"]').val(
                unnumber_format($('input[name="data[UNIT_PRICE]"]').val())
            );
        }
        ...
        $.post("/ajax/popupinsert", { params: param }, function (d) {
            var data = eval("(" + d + ")");
            ...
            if (!data.error) {
                ...
                if (type == "customer") {
                    ...
                    $("#SETCUSTOMER")
                        ...
                        .val(data.Customer.CST_ID);
                } else if (type == "item") {
                    $(".popupSelect" + type).append(
                        $("<option>")
                            .attr({ value: data.Item.ITM_ID })
                            .text(data.Item.ITEM)
                    );
                    ...
                    $("#SETCUSTOMERCHARGE")
                        .children("input[type=text]")
                        .val(data.CustomerCharge.CHARGE_NAME);
                    $("#SETCUSTOMERCHARGE")
                        .children("input[type=hidden]")
                        .val(data.CustomerCharge.CHRC_ID);
                    $("#SETCCUNIT")
                        .children("input[type=text]")
                        .val(data.CustomerCharge.UNIT);
                }
            } else {
                //失敗時
                if (data.error.NAME) {
                    $("#popup")
                        .find("tr.popup_cname")
                        .after(
                            '<tr class="popup_error"><th></th><td><span class="must">' +
                                data.error.NAME +
                                "</span></td></tr>"
                        );
                ...
                if (data.error.NAME_KANA) {
                    $("#popup")
                        .find("tr.popup_ckname")
                        .after(
                            '<tr class="popup_error"><th></th><td><span class="must"><span class="must">' +
                                data.error.NAME_KANA +
                                "</span></td></tr>"
                        );
                ...
                if (data.error.POSTCODE1 || data.error.POSTCODE2) {
                    if (data.error.POSTCODE1) {
                        $("#popup")
                            .find("tr.popup_postcode")
                            .after(
                                '<tr class="popup_error"><th></th><td><span class="must">' +
                                    data.error.POSTCODE1 +
                                    "</span></td></tr>"
                            );
                ...
                if (data.error.ITEM_KANA) {
                    $("#popup")
                        .find("tr.popup_item_kana")
                        .after(
                            '<tr class="popup_error"><th></th><td><span class="must">' +
                                data.error.ITEM_KANA +
                                "</span></td></tr>"
                        );
                ...
                if (data.error.ITEM_CODE) {
                    $("#popup")
                        .find("tr.popup_item_code")
                        .after(
                            '<tr class="popup_error"><th></th><td><span class="must">' +
                                data.error.ITEM_CODE +
                                "</span></td></tr>"
                        );
                ...
                if (type == "item") {
                    $('input[name="data[UNIT_PRICE]"]').val(
                        number_format($('input[name="data[UNIT_PRICE]"]').val())
                    );
                }
            }
        });
