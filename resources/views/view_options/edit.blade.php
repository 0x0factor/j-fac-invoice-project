...
@section('content')
    ...
    <div id="guide">
        <div id="guide_box" class="clearfix">
            ...
            <p>こちらのページはデザイン設定編集の画面です。@if ($user['AUTHORITY'] == 0)
                    <br />必要な情報を入力の上「保存する」ボタンを押下するとデザイン設定を変更できます。
                @endif
            </p>
        </div>
    </div>
    ...
    <div id="contents">
        ...
        <div class="contents_box">
            ...
            <div class="contents_area">
                <form action="{{ route('view_option.update') }}" method="POST" enctype="multipart/form-data"
                    class="ViewOption">
                    @csrf
                                ...
                                <td width="550px">
                                    @if ($option_name === 'logo')
                                        @if (!empty($option_value))
                                            <img src="{{ asset('cms/i_logo.jpg') }}" height="40" id="default_logo"
                                                style="display: none" alt="Default Logo">
                                            <img src="{{ asset('cms/' . $option_value) }}" height="40" id="logo_image"
                                                alt="Logo Image">
                                        ...
                                        @if ($option_value !== 'i_logo.jpg')
                                            <a href="#" id="logo_reset"
                                                onclick="return set_default('{{ $option_name }}');">
                                    ...
                                    @else
                                        <input type="text" name="{{ $option_name }}"
                                            value="{{ old($option_name, $option_value) }}"
                                            class="w600{{ $errors->has($option_name) ? ' error' : '' }}" maxlength="100">
