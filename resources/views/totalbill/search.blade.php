...
<div id="contents">
    ...
    <div class="search_box">
        <div class="search_area">
            <form method="post" action="{{ route('totalbills.search') }}" class="Totalbill">
                @csrf
    ...
    @if(isset($billlist) && is_array($billlist))
        <div class="listview hidebox">
            ...
            <div class="contents_box mb40">
                ...
                <div class="list_area">
                    <form method="post" action="{{ route('totalbills.add') }}" class="Totalbill">
                        @csrf
                        <table width="900" cellpadding="0" cellspacing="0" border="0">
                            ...
                            @foreach($billlist as $i => $val)
                                <tr>
                                    <td>
                                        <input type="checkbox" name="MBL_ID[{{ $val['Bill']['MBL_ID'] }}]" class="chk" id="check_{{ $i }}" value="{{ $val['Bill']['CHK'] ? '1' : '0' }}">
                                    </td>
                                    <td>{{ $val['Bill']['NO'] ?? '　' }}</td>
                                    ...
                                    <div id="subtotal{{ $i }}" style="display:none;">{{ $val['Bill']['SUBTOTAL'] }}</div>
                                </tr>
                            @endforeach
                        </table>
                </div>
                            ...
                            <td width="320">
                                <select name="EDIT_STAT" id="EDIT_STAT">
                                    @foreach($edit_stat as $key => $value)
                                        <option value="{{ $key }}" {{ old('EDIT_STAT', $totalbill->EDIT_STAT) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
