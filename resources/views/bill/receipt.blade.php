...
@section('content')
    ...
    <div id="contents">
        <form action="{{ route('receipt') }}" method="POST" class="Receipt">
            ...
            <h3>
                <div class="edit_01_receipt"><span class="edit_txt">&nbsp;</span></div>
            </h3>
                    ...
                    <table width="880" cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            <th{{ isset($error['CST_ID']) ? ' class="txt_top"' : '' }}>顧客名
                                ...
                                <td width="320">
                                    <input type="text" name="CST_ID" value="{{ $companys }}"
                                        class="w180 p2{{ isset($error['CST_ID']) ? ' error' : '' }}" maxlength="60">
                                ...
                                <th{{ isset($error['TOTAL']) ? ' class="txt_top"' : '' }}>金額
                                    ...
                                    <td width="320">
                                        <input type="text" name="TOTAL"
                                            class="w180 p2{{ isset($error['TOTAL']) ? ' error' : '' }}" readonly
                                            maxlength="16">
                                        <br><span class="must">{{ isset($error['TOTAL']) ? $error['TOTAL'] : '' }}</span>
                                    </td>
                        </tr>
                        <tr>
                            <td colspan="4"><img src="{{ asset('img/i_line_solid.gif') }}"></td>
                        </tr>
                        <tr>
                            <th>発行日</th>
                            <td width="320">
                            ...
                            <th{{ isset($error['RECEIPT_NUMBER']) ? ' class="txt_top"' : '' }}>番号
                                ...
                                <td width="320">
                                    <input type="text" name="RECEIPT_NUMBER"
                                        class="w180 p2{{ isset($error['RECEIPT_NUMBER']) ? ' error' : '' }}"
                                        maxlength="20">
                                    <br><span class="usernavi">{{ $usernavi['RECEIPT_NUMBER'] }}</span>
                                    <br><span
                                        class="must">{{ isset($error['RECEIPT_NUMBER']) ? $error['RECEIPT_NUMBER'] : '' }}</span>
                                </td>
                        </tr>
                        <tr>
                            <td colspan="4"><img src="{{ asset('img/i_line_solid.gif') }}"></td>
                        ...
                        <tr>
                            <th{{ isset($error['PROVISO']) ? ' class="txt_top"' : '' }}>但書き
                                ...
                                <td colspan="3">
                                    <input type="text" name="PROVISO"
                                        class="w440 mr10{{ isset($error['PROVISO']) ? ' error' : '' }}" maxlength="40">
