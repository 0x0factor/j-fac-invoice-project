    ...
    <div class="contents_area4" style="display:none">
        <table width="880" cellpadding="0" cellspacing="0" border="0">
                    ...
                    @if ($action == 'edit')
                        <select name="STATUS" class="form-control">
                            @foreach ($issuedStatCode as $value => $label)
                                <option value="{{ $value }}"
                                    {{ $value == old('STATUS', $status) ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="DEADLINE" value="{{ old('DEADLINE') }}"
                        class="w320 mr10{{ $errors->has('DEADLINE') ? ' error' : '' }}" maxlength="40"
                        onkeyup="count_strw('deadline_rest', this.value, 20)">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="DEAL" value="{{ old('DEAL') }}"
                        class="w320 mr10{{ $errors->has('DEAL') ? ' error' : '' }}" maxlength="40"
                        onkeyup="count_strw('deal_rest', this.value, 20)">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="DELIVERY" value="{{ old('DELIVERY') }}"
                        class="w320 mr10{{ $errors->has('DELIVERY') ? ' error' : '' }}" maxlength="40"
                        onkeyup="count_strw('delivery_rest', this.value, 20)">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="MEMO" value="{{ old('MEMO') }}"
                        class="w320{{ $errors->has('MEMO') ? ' error' : '' }}" maxlength="100"
