 ...
 */
	function input($fieldName, $options = array('div' => false, 'label' => false)) {
 ...
 */
	...
	function dateTime($modelName, $fieldName, $selected = null, $no = null){
		...
		if(isset($this->data[$modelName][$fieldName]) && $this->data[$modelName][$fieldName]){
			$date = $this->data[$modelName][$fieldName];
			if(isset($date['year']))	$selected_year  = $date['year'];
			if(isset($date['month']))	$selected_month = $date['month'];
			if(isset($date['day']))		$selected_day   = $date['day'];
		}else if(is_array($selected)){
			if(isset($selected['year']))	$selected_year  = $selected['year'];
			if(isset($selected['month']))	$selected_month = $selected['month'];
			if(isset($selected['day']))		$selected_day   = $selected['day'];
		}else{
		...
		//年の情報
		$output  = $this->input('name', array('type' => 'select', 'name'  => 'data['.$modelName.']['.$fieldName.'][year]', 'label' => false, 'div' => false, 'options' => $year, 'selected' => $selected_year, 'class'=>'date', 'id'=>'year'.$no));

		$output .= " 年 ";

		//月の情報
		$output .= $this->input('name', array('type' => 'select', 'name'  => 'data['.$modelName.']['.$fieldName.'][month]', 'label' => false, 'div' => false, 'options' => $month, 'selected' => $selected_month, 'class'=>'date', 'id'=>'month'.$no));

		$output .= " 月 ";

		//日の情報
		$output .= $this->input('name', array('type' => 'select', 'name'  => 'data['.$modelName.']['.$fieldName.'][day]', 'label' => false, 'div' => false, 'options' => $day , 'selected' => $selected_day, 'class'=>'date', 'id'=>'day'.$no));
	...
	//配列用inputメソッド
	function inputarray($modelName, $fieldName, $no, $type = null, $attributes = array()){

		//初期データの取得
		$value = null;
		if(isset($this->data[$no][$modelName][$fieldName]) && ($this->data[$no][$modelName][$fieldName] || $this->data[$no][$modelName][$fieldName] == 0)){
			$value = $this->data[$no][$modelName][$fieldName];
		}
		...
		if(is_array($attributes)){
			...
			if(!isset($attributes['name'])){
				$attributes['name'] = 'data['.$no.']['.$modelName.']['.$fieldName.']';
			}
			//valueの設定
			if(!isset($attributes['value']) && ($value || $value == 0)){
		...
		if($type === 'hidden'){
			$output = $this->hidden('name', $attributes);
		}else if($type === 'checkbox'){
			$output = $this->checkbox('name', $attributes);
		}else if($type === 'radio'){
			$output = $this->input('name', $attributes);
		}else if($type === 'select'){
			$output = $this->input('name', $attributes);
		}else{
